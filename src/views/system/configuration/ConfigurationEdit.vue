            ...
            <el-col>
              <el-form-item>
                <el-button type="primary" icon="el-icon-circle-check" @click="submit" :loading="control.loading" v-waves
                           v-has-permission="'configuration:update'">
                  {{$t('operate.save')}}
                </el-button>
...
<script>
import {
  getConfiguration, configKeyExist, updateConfiguration
...
export default {
  ...
  data() {
    ...
    const validateConfigKey = (rule, value, callback) => {
      ...
      // 验证配置键是否重复
      configKeyExist(value, this.id).then(data => {
        if (data.result) {
    ...
    submit() {
      this.$refs.configuration.validate(valid => {
        if (valid) {
          ...
          updateConfiguration(this.configuration).then(data => {
