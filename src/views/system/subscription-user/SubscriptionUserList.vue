          ...
          <div class="in-query-body">
            <el-form ref="parameter" :model="parameter" @keydown.enter.native="query">
              <el-row :gutter="20">
                ...
                <el-col :xs="{span: 12}" :sm="{span: 8}" :md="{span: 6}" :lg="{span: 4}" :xl="{span: 3}"
                        class="in-query-item">
                  <el-select v-model="parameter.subscribed" filterable clearable placeholder="关注状态" style="width: 100%">
                    <el-option v-for="item in control.subscribes" :key="item.value"
                               :label="item.label" :value="item.value"></el-option>
                  </el-select>
                </el-col>
              </el-row>
      ...
      <div class="in-data-container">
        ...
        <div class="in-data-body" style="margin: 15px">
          <el-table v-loading="control.listLoading" :data="tableList" fit highlightCurrentRow
            ...
            <el-table-column prop="subscribed" label="关注状态" sortable="custom">
              <template slot-scope="scope">{{scope.row.subscribedLabel}}</template>
            </el-table-column>
              ...
              <template slot-scope="scope">
                <el-dropdown trigger="click">
                  ...
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item @click.native="info(scope.row.id)" v-has-permission="'subscription-user:info'">
...
export default {
  ...
  methods: {
    ...
    getSubscribes() {
      getDataDictionaries({
        'clazz': 'SubscriptionUser',
        'variable': 'subscribed'
      }).then(data => {
        this.control.subscribes = data
      }).catch(() => {
      })
    },
    // 查询用户列表
    query() {
      ...
      getSubscriptionUsers(this.parameter, this.pager).then(data => {
